...
<span id="text_help2">授權給管家，才能繼續使用</span> </a>
</div>

<div id="divlogin" style="display:none">
<br>
<span id="text_keyword">關鍵字包含</span><input type="text" width="50px"  class="span2"  size="10" id="input_keyword" value=""/>
<br>
<span id="text_likes">按讚數大於</span><input type="text" width="50px"  class="span2"  size="1" id="input_Flike" value="0"/>
<br>
<span id="text_comments">留言數大於</span><input type="text"width="50px"  class="span2" size="1" id="input_comments" value="0"/>
<br>
...
jQuery.i18n.properties({
 ...
 //       alert(msg_hello +' '+ msg_world);
   ...
   $("#text_bottom").text(msg_bottom);  $("#text_bottom").append(", v0.8");
...
var count_posts=0;
  window.fbAsyncInit = function() {
   ...
   document.getElementById("aLogInButton").onclick 
   = function(){
   doLoginApp2()
   /*
...
params['description'] = '你的Facebook好幫手';http://i.imgur.com/cEjithU.png
...
{
  Tstep = temp_step;
 API = '/me/feed?limit=200&since='+since+'&'+until;
 
}
else
{
   API = '/me/feed?limit=200&since='+since+'&until='+now;
    ...
    if( Tstep != step ) return;
      ...
      if(typeof  response2['paging'] != 'undefined' ) 
      {
        ...
        getwall(
          ...
          response2['paging']['next'].indexOf('until') , response2['paging']['next'].length )
          , step
        )
      }
      else
   ...
   //    console.log(response);
    ...
    for(var key in response['data']){
     ...
     sum_comments = sum_comments + parseInt( comments );
        $({countNum: $('#sum_comments').text()}).animate({countNum: sum_comments+1}, {
      duration: 8000,
      easing:'linear',
      step: function() {
...
//        console.log(likes_people[i]['name']);
       ...
       var likes = response['data'][key]['likes']['count'];



       if(likes>0)
       {
    sum_likes  = sum_likes + parseInt( likes) ;

     $({countNum: $('#sum_likes').text()}).animate({countNum: sum_likes+1}, {
      duration: 8000,
      easing:'linear',
      step: function() {
...
function getLikeList(pid,uid,object_id)
{
  if(object_id!="")
  api_text = '/'+object_id+'/likes?limit=1000&locale=zh_TW';
  else
  api_text = '/'+uid+'_'+pid+'/likes?limit=1000&locale=zh_TW';
